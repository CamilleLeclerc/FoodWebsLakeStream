...
## 07-07-2022 - created model_1_v0_3
##            - removed dbo^2 from model
...
X_obs   = cbind(1,year,temp,temp^2,type,type*temp,dbo,type*dbo,temp*dbo,alt,rich)
X_mis   = cbind(1,year,temp,temp^2,type,type*temp,  1,type*1  ,  temp*1,alt,rich)
n_beta  = ncol(X_obs)
...
{
    X_mis[,7:9] = X_mis[,7:9] * cbind(x_mis,x_mis,x_mis)
...
for(i in 1:2)
{
    ...
    X_pred   = cbind(1,0,0,0,(i-1),0,x,(i-1)*x,0,0,0)
    pred     = chainList.apply(chainList_thinned,function(x_) Yhat(X_pred,x_[-1][n_sd_lik + 1 + 1 + 1:n_beta]))
...
colnames(chain_) = c("P","1","year","temp","temp^2","type","type*temp","dbo","type*dbo","temp*dbo","alt","rich")
