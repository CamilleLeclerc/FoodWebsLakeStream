...
## goal: function to add error around model prediction on a plot using the polygon function

## args
# @X - vector - vector of x values
# @y - vector - vector of y values
# @sigma - float - value of the error around model prediction

##
polygon.gamma <- function(X,Y)
{
  polygon(x=c(X,rev(X)), y=c(qgamma(p=0.05, shape=Y, rate=1), rev(qgamma(p=0.95, shape=Y, rate=1))), col=adjustcolor("grey",alpha=0.2), border=NA);
}

polygon.truncnorm <- function(X,Y,sigma)
{
  polygon(x=c(X,rev(X)), y=c(qtruncnorm(p=0.05, 0, Inf, Y, sigma), rev(qtruncnorm(p=0.95,0,Inf,Y,sigma))), col=adjustcolor("grey",alpha=0.2), border=NA);
}
...
## goal: takes a list of chains and return critical summary statistics
...
chainList.bayesPlot <- function(chainList, logTransform=F)
{
