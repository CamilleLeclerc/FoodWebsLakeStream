...
```{r load}
myload(ind_size, metaweb_lake, metaweb_analysis, length_analysis, dir = mypath("data"))
```
...
lake_of_interest <- read.csv(paste0(mypath("outputs"), "/final_lake_list.txt"), sep="")
ind_size %<>% filter(id_campagne %in% unique(lake_of_interest$id_campagne))
...
weight_fish_lake %<>% filter(id_campagne %in% unique(lake_of_interest$id_campagne))
