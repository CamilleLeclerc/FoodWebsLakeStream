...
```
```{r}
ind_size %<>% rename(species_name = species)
...
network_lake <- build_local_network(
  data = filter(ind_size, species_name %in% metaweb_lake$size_class$species_name),
...
size_class_weight_lake <- assign_size_class(
  data = weight_fish_lake %>%
    filter(!is.na(fish),
      species %in% metaweb_lake$size_class$species_name),
