...
library(sf)
myload(region_polygon, op_analysis, station_analysis, dir = mypath("data"))
# get the map
...
op_analysis_to_merge <- op_analysis %>%
  ungroup() %>%
  distinct(opcod, year) %>%
  rename(id_campagne = opcod, camp_annee = year) %>%
  mutate(type = "stream")
sampling_lake_stream <- sampling_protocol %>%
  distinct(id_campagne, camp_annee) %>%
  mutate(type = "lake") %>%
  rbind(., op_analysis_to_merge)

sampling_lake_stream %>%
  ggplot(aes(x = camp_annee)) +
  geom_histogram() +
